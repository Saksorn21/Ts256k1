...
describe('class main Ts256k1 implements Service', () => {
  ...
  it('should throw an error if private key is invalid (not 32 bytes)', () => {
    ...
    expect(() => Ts256k1.getKeyPairs(randomBytes(10))).toThrow(
      'Invalid private key'
    )
  })
  it('should return valid key pair from static getKeyPairs method', () => {
    ...
    expect(Uint8Array.from(hexToBytes(pkHexcom))).toEqual(
      Uint8Array.from(pk.compressed)
    )
    expect(Uint8Array.from(hexToBytes(pk.toHex(false)))).toEqual(
      Uint8Array.from(pk.uncompressed)
    )
  })
...
describe('equals method in Ts256k1 class', () => {
  ...
  it('should throw TypeError for invalid comparison type', () => {
    expect(() => k1.equals({} as PrivateKey)).toThrow(
      `Type mismatch: Expected instance of PrivateKey or PublicKey, but got object.`
    )
  })
  ...
  it('should return false for unequal public keys', () => {
    const differentPublicKeyInstance = new PublicKey(
      K1.getPublicKey(randomBytes(32))
    )
