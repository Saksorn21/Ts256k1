...
describe('Decryption Tests', () => {
  ...
  beforeEach(() => {
    privateKey = new PrivateKey(randomBytes(config.ConstsType.SECRET_KEY_LENGTH));
    encryptedMessage = Uint8Array.from(encrypt(privateKey.publicKey.toHex(), message))
  })
...
it('should correctly decode the signed message', () => {
    ...
    const signedMessage = Uint8Array.from(Buffer.concat([signature, encryptedMessage]))
  ...
  test('should extract parts from cipher text', () => {
    const cipherText = new Uint8Array([...randomBytes(24), ...new Uint8Array(16), ...encryptedMessage]);
