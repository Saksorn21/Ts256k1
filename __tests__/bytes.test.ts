...
describe('bytes', () => {
  ...
  it('ucs2 test', () => {
    ...
    expect(ucs2ToBytes(bytesToUcs2(hello))).toEqual(
      ucs2ToBytes(bytesToUcs2(hello))
    )
  })
  ...
  describe('abytes', () => {
    ...
    it('should throw a TypeError for a string', () => {
      ...
      expect(() => abytes('hello')).toThrow(
        'Expected Uint8Array or Buffer, got string'
      )
    })

    it('should throw a TypeError for null', () => {
      expect(() => abytes(null)).toThrow(TypeError)
      expect(() => abytes({})).toThrow(
        'Expected Uint8Array or Buffer, got object'
      )
    })

    it('should throw a TypeError for undefined', () => {
      expect(() => abytes(undefined)).toThrow(TypeError)
      expect(() => abytes(undefined)).toThrow(
        'Expected Uint8Array or Buffer, got undefined'
      )
    })
