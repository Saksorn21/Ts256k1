...
function _encrypt(key, plainText) {
    ...
    const nonce = (0, utils_1.randomBytes)(config_1.ConstsType.XCHACHA20_NONCE_LENGTH);
    const cipher = (0, utils_1.xchacha20)(key, nonce);
    const ciphered = cipher.encrypt(plainText);
    const encrypted = ciphered.subarray(0, ciphered.length - config_1.ConstsType.AEAD_TAG_LENGTH);
    const tag = ciphered.subarray(ciphered.length - config_1.ConstsType.AEAD_TAG_LENGTH);
 ...
 */
function signMessage(encryptedData, privateKey) {
    const messageHash = (0, utils_1.sha256)(encryptedData);
    const signature = utils_1.K1.sign(messageHash, privateKey, {
        lowS: (0, config_1.isSignatureOptsLowS)(), //configuration (file: ts256k1.config.json))
 ...
 */
function encodeSign(encryptedMessage, privateKey) {
    ...
    return Buffer.from((0, utils_1.concatBytes)(createSignature, encryptedMessage));
}
