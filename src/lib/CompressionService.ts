...
export class CompressionService {
  ...
  static extractDictionary(data: Uint8Array, coverage: CompressOpts['coverage'] = 2 ): Uint8Array {
...
this.preloadAllCacheFiles(this.cacheDir);
  ...
  private preloadOneCacheFile(prefix: string, cache: Map<string, CacheEntry>): void {
    const raw = analyzeFiles(this.cacheDir, utf8ToBytes(prefix))[prefix]
    cache.set(prefix, { PREFIXUUID: prefix, totalSize: raw.totalSize, dataStorage: raw.dataStorage });
      
    
}
    private preloadAllCacheFiles(dir: string): void {
 for (const [key,value] of Object.entries(analyzeFiles(dir))){
   const { prefix, totalSize, dataStorage} = value as RawPreload[keyof RawPreload]
     this.cache.set(key,
       ({ 
         PREFIXUUID: prefix,
         totalSize: totalSize, 
  ...
  public compress(data: Uint8Array, options: CompressOpts) {
    ...
    const dictionary = CompressionService.extractDictionary(data, options.coverage);
    const { totalSize } = calculateChunkSize([data], data.length)
    const prefixUUID = writeFileBit(dictionary, this.cacheDir, this.UUID,totalSize);
  ...
  public decompress(data: Uint8Array, options?: InflateOptions): Uint8Array{
    ...
    const prefix = bytesToUtf8(data.subarray(0, ConstsType.PREFIX_COMPRESS_LENGTH))

      if (this.cache.has(prefix) === undefined ) {
        this.preloadOneCacheFile(prefix, this.cache);
      }
    
      const { PREFIXUUID, totalSize, dataStorage } = this.cache.get(prefix) as CacheEntry; 
    
    if (prefix !== PREFIXUUID){
throw new Error(`Prefix not match ${prefix}, got ${PREFIXUUID}`)
    }
      const decompressedData = decompressData(data.subarray(ConstsType.PREFIX_COMPRESS_LENGTH), {
  ...
  public compressSync( options: DeflateOptions, ...data: Uint8Array[]): Uint8Array{
