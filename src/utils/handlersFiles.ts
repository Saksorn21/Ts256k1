...
export const UUID = (): string => `${Math.random().toString(16).substring(2, 10)}`;
...
export function writeFileBit(data: Uint8Array, dir: string, uuid: string, dataCount: number): Uint8Array {
  const PREFIX = uuid;
...
export function findFilesByPrefix(prefix: Uint8Array, dir: string): string | undefined {
  try {
...
export function analyzeFiles(dir: string, PREFIX?: Uint8Array): RawPreload {
  ...
  const fileCache = PREFIX ? findFilesByPrefix(PREFIX, dir) : listCacheFiles(dir);
 ...
 * ประมวลผลหลายไฟล์
 */
function processMultipleFiles(fileCache: string[], dir: string, result: RawPreload): void {
 ...
 * ประมวลผลไฟล์เดี่ยว
 */
function processSingleFile(file: string, dir: string, result: RawPreload): void {
  const data = readFileBit(file, dir);
  const { prefix, totalSize } = extractFileInfo(file);
  result[prefix] = {
    prefix,
 ...
 */
function extractFileInfo(fileName: string): { prefix: string, totalSize: number } {
